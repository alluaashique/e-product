...
<main class="h-full pb-16 overflow-y-auto">
    <div class="container px-6 mx-auto grid">
        ...
        <form id="myForm" method="POST" action="{{route('admin.product-category.update',$product_category->id)}}" enctype="multipart/form-data">
        @method("PUT")
        @else
        <form id="myForm" method="POST" action="{{route('admin.product-category.store')}}" enctype="multipart/form-data">
        @method("POST")
        @endif
            @csrf
            ...
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                ...
                <label class="block text-sm">
                    ...
                    type="text" id="product_category" name="product_category" placeholder="Product Category"
                    value="{{ old('product_category') ?? $product_category->name ?? "" }}" />
                ...
                <label class="block mt-4 text-sm">
                    ...
                    <input type="file" id="image" name="image" class="form-control" accept="image/*" @if(!isset($product_category)) required @endif >
                    @error('image')
                    <span class="text-xs text-red-600 dark:text-red-400"> {{ $message }} </span>                    
                    @enderror
                </label>
                ...
                @if(isset($product_category))
                    <label class="block mt-4 text-sm">
                        ...
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ $product_category->is_active == 1 ? 'checked' : '' }} >
                        @error('is_active')
                        {{ $message }}
                        @enderror
                    </label>              
